...
from .settings import RANKING_METHODS, ELECTION_METHODS, PARTIAL_METHODS, METHODS_SETTINGS, PRETTY_NAMES


class Leaderboard:
    from ._rules import mean_ranking, mean_election, plurality_ranking, plurality_election, \
        threshold_election, borda_ranking, borda_election, dowdall_ranking, dowdall_election, \
        condorcet_election, baldwin_election, copeland_ranking, copeland_election, \
        minimax_ranking, minimax_election, _approval_ranking
    ...
    def __init__(self, table: pd.DataFrame = None, weights: Dict[str, float] = None):
        ...
        self.weights = pd.Series(index=self.tasks, data=1.)
    ...
    def build_ranks(self):
        ...
        wins = {model: ((self.ranks.loc[model] < self.ranks) * self.weights).sum(axis=1) > \
            ((self.ranks.loc[model] > self.ranks) * self.weights).sum(axis=1) for model in self.models}
        ties = {model: ((self.ranks.loc[model] < self.ranks) * self.weights).sum(axis=1) == \
            ((self.ranks.loc[model] > self.ranks) * self.weights).sum(axis=1) for model in self.models}

        self.majority_graph = pd.DataFrame(wins).transpose() + \
            0.5 * pd.DataFrame(ties).transpose() + np.eye(self.n_models) * 0.5
    ...
    def elect_all(self, use_methods: List[str] = None):
        ...
        if self.is_partial:
            methods_to_choose = set(methods_to_choose).intersection(set(PARTIAL_METHODS))
        ...
        for method in methods_to_choose:
            ...
            if method in settings_dict:
                ...
                for some_params in params:
                    result.append((f"Method: {method}, Params: {some_params}", func(**some_params)))
            else:
    ...
    def rank_all(self, task_groups=None, group_weights=None, insert_nan=True, use_methods: List[str] = None):
        result = []
            ...
            if method in settings_dict:
                ...
                for some_params in params:
                    if task_groups is None:
                        ...
                        ranking = self.two_step_ranking(method, task_groups, group_weights=group_weights,
                                                        ranking_params=some_params, insert_nan=insert_nan)
                    to_print = (ranking.apply(lambda x: f"{x:.2f}: ") + ranking.index).values
    ...
    def get_meta_leaderboard(self, ranking_method, task_groups, group_weights=None, ranking_params=None,
                             insert_nan=True):
    ...
    def two_step_ranking(self, ranking_method, task_groups, group_weights=None, ranking_params=None,
                         insert_nan=True):
        meta_lb = self.get_meta_leaderboard(ranking_method, task_groups, group_weights, ranking_params,
                                            insert_nan=insert_nan)
