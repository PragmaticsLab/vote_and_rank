    ...
    with tqdm(total=num_repeats * len(exp_range)) as pbar:
        for repeat_idxs in range(num_repeats):
            ...
            for nan_number in exp_range:
                ...
                nan_idxs_prod = np.random.choice(table_nan.shape[0] * table_nan.shape[1], 
                                                 size=nan_number, replace=False)
                ...
                ranks.append(noised_lb.rank_all(
                    use_methods={"mean": {"mean_type": ["arithmetic"]}, 
            ...
            for cur_rankings in ranks:
                for col in cur_rankings.columns:
                    ...
                    corrs[col].append(spearmanr(etalon[col].loc[topk_methods],
                                                     norm_rank.loc[topk_methods])[0])
...
def create_exp_pic(exp_range, exp_res, filename=None):
    ...
    sns.lineplot(x="criterion", y="correlation", hue="method", style="method", data=df, linewidth = 5, ci=100)
